describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    //console.log(product);
    const property = (await product.getProperty('shadowRoot')).asElement();
    //console.log(property);
    const button = await property.$('button');
    await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++){
      let shadow = (await prodItems[i].getProperty('shadowRoot')).asElement();
      let button = await shadow.$('button');
      await button.click();
    }
    const cartElem = await page.$('#cart-count');
    const cartText = await cartElem.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++){
      const shadow = (await prodItems[i].getProperty('shadowRoot')).asElement();
      let button = await shadow.$('button');
